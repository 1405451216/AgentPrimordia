...
更系统的入门教程：docs/getting-started/（installation / quickstart / first-agent）、docs/guide/react-loop.md；实战配方见 docs/cookbook/。
...
2. standalone：未找到时仅写 require agentprimordia v0.0.0 占位并打印提示——受 Go 语义化导入版本限制，框架必须本地 replace 后才可 tidy（VERSIONING.md 有模块消费限制说明）。
...
- docs/VERSIONING.md —— 模块消费限制与语义化导入版本约束（脚手架 replace 策略依据）
- docs/getting-started/ —— installation / quickstart / first-agent（图文入门三连）
- docs/supply-chain-security.md —— cosign/SBOM 供应链安全
