# 一体化 Coding Harness（计划 → 编写 → 测试 → 审查 → 发布）

> Go 框架线与 TS SDK 线均已打通该流程，API 与行为对齐：同一个 Agent 装配一次，
> 端到端完成任务分解、代码编写、验证测试、自审查与 git 发布。
...
- 系统提示中说明工具职责边界（filesystem 负责编写、shell 负责测试、git 负责发布），降低工具选择歧义。
