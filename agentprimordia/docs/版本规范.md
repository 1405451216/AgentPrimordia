...
当前版本：`6.0.1`（定义于 `pkg/agent.go`，git tag 管理）
...
| 组件 | 当前版本 | 版本定义位置 |
|------|----------|----------------|
| Go SDK | v6.0.1 | `pkg/agent.go` + git tag |
| TypeScript SDK | v6.0.1 | `../../sdk/typescript/package.json`（仓库根 sdk/ 目录） |
| Python 客户端 | v2.0.0 | `../../sdk/python/pyproject.toml` |
| Rust 客户端 | v2.0.0 | `../../sdk/rust/Cargo.toml` |
| CLI | v6.0.1 | `cmd/ap/main.go`（`var Version`，发布经 ldflags 注入） |
...
|-----|------|---------|---------|---------|------|
| 世界模型（WithWorldModel） | opt-in → 默认开启 | v7.0 | docs/evals/long-horizon-v1.json 留出子集（docs/evals/manifest.json 冻结），判据见 docs/提案-世界模型默认策略切换.md §五 | WithoutWorldModel() | **延迟**：留出集数字待 A1 secrets 后 nightly 双臂实验回填；v7.0 保持 opt-in，延迟原因登记于 ecosystem/docs/migration/v7-deprecations.md §一 |
