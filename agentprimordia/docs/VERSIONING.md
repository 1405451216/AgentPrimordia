...
当前版本：`5.0.0`（定义于 `pkg/agent.go`，git tag 管理）
...
| 组件 | 当前版本 | 版本定义位置 |
|------|----------|----------------|
| Go SDK | v5.0.0 | `pkg/agent.go` + git tag |
| TypeScript SDK | v5.0.0 | `sdk/typescript/package.json` |
| Python 客户端 | v2.0.0 | `sdk/python/pyproject.toml` |
| Rust 客户端 | v2.0.0 | `sdk/rust/Cargo.toml` |
| CLI | v5.0.0 | `cmd/ap/main.go`（`var Version`，发布经 ldflags 注入） |
