...
| 当前状态 | **延迟**：留出集数字未回填——nightly 真实轨评测依赖 CI LLM secrets（§九 A1），降级豁免记录见 docs/V7路线图.md §二 S0-1 残余项 |
...
| 构件 | 版本 | 工程地板 | 验收数字 | 依赖 |
|------|------|---------|---------|------|
| 世界模型（具模） | v6.1 | 状态图/六挂钩/state-checkpoint/一致性门/双线对等 ✅ | 命题 1/2/3 待 A1 | A1 secrets |
...
| 常驻运行时（长活） | v6.4 | 自唤醒/预算不变式/自愈/idle 代谢/ap live ✅（14 天模拟 harness） | 命题 1（14 天 ≥99%）/2/3 待 B2 | B2 常驻宿主 |
