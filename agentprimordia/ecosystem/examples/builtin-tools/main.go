...
func demonstrateAgentWithTools() {
	...
	toolAgent := agent.NewReActAgent(agent.ReActConfig{
		Name:         "ToolBot",
	...
	resp1, _ := toolAgent.Run(ctx, agent.UserMessage("请帮我计算 15 + 25 的结果"))
	fmt.Printf("   🤖 回复: %s\n", resp1.Content)

	fmt.Println("\n📝 测试2: 获取当前时间")
	resp2, _ := toolAgent.Run(ctx, agent.UserMessage("现在几点了？请用简单的格式告诉我"))
	fmt.Printf("   🤖 回复: %s\n", resp2.Content)

	fmt.Println("\n📝 测试3: 组合任务")
	resp3, _ := toolAgent.Run(ctx, agent.UserMessage(
