...
func TestPIICheck(t *testing.T) {
	t.Run("all-redacted", func(t *testing.T) {
		...
		if !f.Passed { t.Error("expected pass") }
		if f.Severity != "info" { t.Errorf("severity = %v, want info", f.Severity) }
	})
	...
	t.Run("partial-redacted", func(t *testing.T) {
		...
		f, err := c.Check(context.Background())
		if err != nil { t.Fatal(err) }
		if f.Passed { t.Error("expected fail") }
		if f.Severity != "high" { t.Errorf("severity = %v, want high", f.Severity) }
	})
	...
	t.Run("no-requests", func(t *testing.T) {
		...
		f, err := c.Check(context.Background())
		if err != nil { t.Fatal(err) }
		if !f.Passed { t.Error("expected pass with no requests") }
	})
...
func TestEncryptionCheck(t *testing.T) {
	...
	t.Run("enabled-bad-rotation", func(t *testing.T) {
		...
		f, err := c.Check(context.Background())
		if err != nil { t.Fatal(err) }
		if f.Passed { t.Error("expected fail for long rotation") }
		if f.Severity != "medium" { t.Errorf("severity = %v, want medium", f.Severity) }
	})
	...
	t.Run("disabled", func(t *testing.T) {
		...
		if err != nil { t.Fatal(err) }
		if f.Passed { t.Error("expected fail") }
		if f.Severity != "critical" { t.Errorf("severity = %v, want critical", f.Severity) }
	})
}
...
func TestRetentionCheck(t *testing.T) {
	...
	t.Run("overdue", func(t *testing.T) {
		...
		f, err := c.Check(context.Background())
		if err != nil { t.Fatal(err) }
		if f.Passed { t.Error("expected fail for overdue records") }
	})
}
...
func TestAccessControlCheck(t *testing.T) {
	...
	t.Run("high-deny-rate", func(t *testing.T) {
		...
		f, err := c.Check(context.Background())
		if err != nil { t.Fatal(err) }
		if f.Passed { t.Error("expected fail for high deny rate") }
	})
	...
	t.Run("no-events", func(t *testing.T) {
		...
		f, err := c.Check(context.Background())
		if err != nil { t.Fatal(err) }
		if !f.Passed { t.Error("expected pass with no events") }
	})
}
...
func TestGenerateReport(t *testing.T) {
	t.Run("all-pass", func(t *testing.T) {
		...
		if err != nil { t.Fatal(err) }
		if report.Score != 100.0 { t.Errorf("score = %v, want 100", report.Score) }
		if len(report.Findings) != 3 { t.Errorf("findings = %d, want 3", len(report.Findings)) }
		if report.Framework != SOC2 { t.Errorf("framework = %v, want SOC2", report.Framework) }
		if report.HasCriticalFindings() { t.Error("should not have critical findings") }
		if report.PassedChecks() != 3 { t.Errorf("passed = %d, want 3", report.PassedChecks()) }
	})
	...
	t.Run("partial-pass", func(t *testing.T) {
		...
		if err != nil { t.Fatal(err) }
		if report.Score != 50.0 { t.Errorf("score = %v, want 50", report.Score) }
		if report.PassedChecks() != 2 { t.Errorf("passed = %d, want 2", report.PassedChecks()) }
	})
	...
	t.Run("all-fail", func(t *testing.T) {
		...
		if err != nil { t.Fatal(err) }
		if report.Score != 0.0 { t.Errorf("score = %v, want 0", report.Score) }
		if !report.HasCriticalFindings() { t.Error("should have critical findings") }
	})

	t.Run("empty-checks", func(t *testing.T) {
		report, err := GenerateReport(context.Background(), GDPR, nil)
		if err != nil { t.Fatal(err) }
		if report.Score != 0 { t.Errorf("score = %v, want 0", report.Score) }
		if len(report.Findings) != 0 { t.Errorf("findings = %d, want 0", len(report.Findings)) }
	})
...
func TestAssessmentReport_JSON(t *testing.T) {
	...
	if m["framework"] != "GDPR" { t.Errorf("framework = %v, want GDPR", m["framework"]) }
	_, ok := m["score"].(float64)
	if !ok { t.Error("score should be float64") }

	// 验证 period 字段
	period, ok := m["period"].(map[string]any)
	if !ok { t.Error("period should be object") }
	if _, ok := period["start"]; !ok { t.Error("period.start missing") }

	// 验证 findings 字段
	findings, ok := m["findings"].([]any)
	if !ok || len(findings) != 1 { t.Error("findings should have 1 element") }

	// 验证 data_accesses 字段
	accesses, ok := m["data_accesses"].([]any)
	if !ok || len(accesses) != 1 { t.Error("data_accesses should have 1 element") }
}

func TestComplianceFramework_Constants(t *testing.T) {
	if GDPR != "GDPR" { t.Errorf("GDPR = %v", GDPR) }
	if SOC2 != "SOC2" { t.Errorf("SOC2 = %v", SOC2) }
	if MLPS != "MLPS" { t.Errorf("MLPS = %v", MLPS) }
}
