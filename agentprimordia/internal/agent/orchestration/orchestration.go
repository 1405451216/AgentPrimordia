// Package orchestration 提供多 Agent 编排模式
...
type Hooks interface {
	Fire(ctx context.Context, event string, data map[string]interface{}) error
}
...
func (p *Pipeline) Run(ctx context.Context, initialInput string) (*PipelineResult, error) {
	...
	for i, step := range p.steps {
		...
		if p.hooks != nil {
			_ = p.hooks.Fire(ctx, HookBeforePipelineStep, map[string]interface{}{"step": step.Name, "index": i})
		}

		output, err := step.Agent.Run(ctx, input)
		stepDuration := time.Since(stepStart)

		if p.hooks != nil {
			_ = p.hooks.Fire(ctx, HookAfterPipelineStep, map[string]interface{}{"step": step.Name, "index": i})
		}
...
func (h *Handoff) Run(ctx context.Context, input string) (*HandoffResult, error) {
	...
	for i := 0; i < h.config.MaxHandoffs; i++ {
		...
		if h.hooks != nil {
			_ = h.hooks.Fire(ctx, HookBeforeHandoff, map[string]interface{}{"agent": agent.Name(), "handoff": i})
		}
		...
		if h.hooks != nil {
			_ = h.hooks.Fire(ctx, HookAfterHandoff, map[string]interface{}{"agent": agent.Name(), "handoff": i})
		}
...
func ParallelRun(ctx context.Context, agents []Agent, input string, hooks Hooks) (*ParallelResult, error) {
	...
	for i, a := range agents {
		...
		go func(idx int, agent Agent) {
			...
			if hooks != nil {
				_ = hooks.Fire(ctx, HookBeforeParallelAgent, map[string]interface{}{"agent": agent.Name(), "index": idx})
			}
			...
			if hooks != nil {
				_ = hooks.Fire(ctx, HookAfterParallelAgent, map[string]interface{}{"agent": agent.Name(), "index": idx})
			}
