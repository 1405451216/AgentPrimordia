...
func TestHTTPTransport_DoubleStart(t *testing.T) {
	tr := NewHTTPTransport()
...
func TestHTTPTransport_ConcurrentMessages(t *testing.T) {
	sender := NewHTTPTransport()
	receiver := NewHTTPTransport()
...
func TestHTTPTransport_LargeMessage(t *testing.T) {
	sender := NewHTTPTransport()
	receiver := NewHTTPTransport()
...
func TestHTTPTransport_SendBeforeStart(t *testing.T) {
	tr := NewHTTPTransport()
...
func TestHTTPTransport_CloseBeforeStart(t *testing.T) {
	tr := NewHTTPTransport()
...
func TestHTTPTransport_MetadataPreserved(t *testing.T) {
	sender := NewHTTPTransport()
	receiver := NewHTTPTransport()
	...
	select {
	case received := <-receiver.Receive():
		if received.Metadata["priority"] != "high" {
			t.Errorf("metadata priority = %q, want %q", received.Metadata["priority"], "high")
		}
		if received.Metadata["source"] != "test" {
			t.Errorf("metadata source = %q, want %q", received.Metadata["source"], "test")
		}
	case <-time.After(3 * time.Second):
		t.Fatal("timeout waiting for message")
	}
