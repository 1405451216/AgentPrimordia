...
func (n *AgentDelegateNode) Stats() AgentStats { return n.agent.Stats() }

func (n *AgentDelegateNode) Run(ctx context.Context, msg Message) (*Response, error) {
	return n.agent.Run(ctx, msg)
}

func (n *AgentDelegateNode) StreamRun(ctx context.Context, msg Message) (<-chan StreamEvent, error) {
...
func (n *SubWorkflowNode) Stats() AgentStats { return AgentStats{} }

func (n *SubWorkflowNode) Run(ctx context.Context, msg Message) (*Response, error) {
	...
	return &Response{Content: strings.TrimSpace(output.String())}, nil
}

func (n *SubWorkflowNode) StreamRun(ctx context.Context, msg Message) (<-chan StreamEvent, error) {
	resp, err := n.Run(ctx, msg)
	if err != nil {
		return nil, err
	}
	ch := make(chan StreamEvent, 1)
	ch <- StreamEvent{Type: StreamEventComplete, Content: resp.Content}
	close(ch)
	return ch, nil
}
...
// DelegateNode 添加 Agent 委派节点
func (b *DAGBuilder) DelegateNode(id string, agent Agent) *DAGBuilder {
	if b.err != nil {
		return b
	}
