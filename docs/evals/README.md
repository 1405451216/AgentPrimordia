...
> 验收只认已注册计划书的分析口径（模板见 [`../实验成本与功效模板.md`](../实验成本与功效模板.md)）；
> **一题面一注册口径**：修改假设/指标/N 须修订重新注册并在 CHANGELOG 记录，禁止静默变更。

| 文件 | 命题 | 主判据 | 样本量口径 |
|------|------|--------|-----------|
| `plans/v6.1-命题1-世界模型配对AB.md` | v6.1 命题 1（model-based 规划） | 双臂配对成功率 +≥15pp 且 McNemar p<0.05；判定走 `asserts.go` 确定性断言（禁 LLM judge） | `SampleSizeMcNemar(0.15,0.30)=108` → 4 整轮 96 + 第 5 轮 12 = 108 配对 = 216 次运行；成本 4.7×10⁷ token 走 B4 |
| `plans/v6.1-命题2-状态追踪回放.md` | v6.1 命题 2（状态追踪可信） | n=100 人工标注，点估计 ≥90% 且 Wilson 下界 ≥82%；100/100 须写 Wilson [96.3%,100%] 禁裸 100%（R3） | n=100（等价式失败 ≤10） |
| `plans/v6.1-命题3-记忆状态层.md` | v6.1 命题 3（记忆=状态层） | 双门：闭环覆盖 24/24（确定性门，唯一允许 100%）+ 旧文本路径一致性 n=300 ≥99%（失败 ≤3） | n=300 |

**题面引用（三份统一）**：`long-horizon-v1.json` @commit `027b5c56542783dab601558eb99ced10b0458277`（sha256 `246aba33…`，留出 9/24）。

**注册史注记**：S0-6 收口曾有并行会话竞争稿（p1/p2 短稿、p3 n=100 对口径），按「一题面一注册口径」归一于 97223fd7 正式三份——p1/p2 删除、p3 存 `plans/archive/`（带废弃说明）。
**台账防回退注记**：`freeze_commit` 登记于 d0491cbe（027b5c56 = 题面数据 commit），e7995ba6 重算台账时曾冲回 PENDING 占位；687c77a7 恢复登记值并加两道防回退——`eval-manifest.py --write` 未显式传 `--freeze-commit` 保留既有值；冻结门 `VerifyFrozen` 将空/PENDING 列为违规（回归测试 `TestVerifyFrozenRequiresFreezeCommit`）。
