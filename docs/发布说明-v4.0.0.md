...
v4.0.0 是 v4.0 路线的收官版本：四个能力跃迁全部落地（v3.3 长期自治 / v3.4 技能进化 / v3.5 协议互操作 A2A Open / v3.6 多模态实时，见 `docs/V4路线图.md`），实证路线 v3.3→v4.0 的 35 项声称经 2026-08-09 深度评估逐一复核（33 项与代码相符，见 `docs/路线图.md` 与 `agentprimordia/docs/项目评估-2026-08-09.md`），并固化发布纪律（tag 自动化 + 版本机器门）。
...
- `WithSkills` + `pkg/skills.go` + CLI `ap skill`；文档 `docs/guides/技能格式.md`
...
- `pkg/a2a_interop.go` + CLI `ap a2a interop-check`；文档 `docs/guides/A2A互通.md`
...
- 文档 `docs/guides/实时通信.md`；验收 demo `ecosystem/examples/realtime-voice/`
...
- `pkg/version_gate_test.go`：强制校验版本格式合法（语义化版本）且与 版本规范.md 一致（漂移即失败）
- 版本 bump 至 4.0.0：Go SDK / TS SDK / CLI / VERSIONING / api-contract 全线对齐

## 破坏性变更（v4.0.0）

主版本升级按 版本规范.md「已移除记录」清理全部超期废弃 API：
...
- 稳定 API 清单与兼容性承诺：`agentprimordia/docs/版本规范.md`
