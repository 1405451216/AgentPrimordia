...
| v5.0 | 稳定 | 平台里程碑 | 一键部署 + 迁移指南就绪 + 评估复测 ≥8.5 | ✅ 已完成（2026-08-09：dev-platform 一键部署脚本；全能力整合 e2e 套件 `test/e2e/all_capabilities_test.go`；stability 新导出标注+门通过；v5 迁移指南 `ecosystem/docs/migration/v5-deprecations.md`；版本 5.0.0 四方一致（Go/TS/VERSION/api-contract）+ version 门全绿；评估复测 ≈8.8/10 ≥8.5；Studio 压测（`bench/suite/studio_load_test.go` + `bench/soak/studio_soak_test.go`：30 分钟稳态 88182 请求 0 错误无退化，检出并修复 demo 存储无界膨胀）） |
