...
- [x] §一 现状红线证据（R1–R6）复核：ErrImprovementScopeViolation 全仓仅定义/抛出/测试三处的核实结论认可
- [x] §二 INV-0 表述认可；A1–A8 断言逐条可测（检测点/实现位置/穷尽方式成立）
- [x] §2.3 100%/0 容忍的 R3 论证成立（判定为算法 + 测试面可枚举 + 失效显式）
- [x] §三 释放/不释放边界与已知缺口四条如实披露（含两条标注推断待证项）认可
- [x] §四 题面只读承诺 + sha256/family 数字与 docs/evals/manifest.json 一致
- [x] §五 统一 cosign 口径 + 信任锚 + 三关注册前置认可；Acquire 验签接线待证项已如实标注
- [x] §六 三级回滚与事故响应（全量披露+根因复审）成立
- [x] §七 流程条款认可并作为 AGENTS.md §2.3 落地
- [x] §八 四处 AGENTS.md 修订稿可原文落地

**v6.3 功能代码合并前置（先于任何生成链路代码）**

- [x] AGENTS.md 四处修订 commit 先行合入
- [x] A1–A8 断言测试全绿并进 CI
- [x] adversarial-holdout-v1.json 留出子集回归全过（eval-manifest.py --verify 对账）
...
| 日期 | 结论 | 意见摘要 | 批准人 |
|------|------|---------|--------|
| 2026-08-31 | **批准** | 维护者书面批准（会话原话「全部批准，都交给你」）；生效要件第二项（红队样本集回归全过：adversarial-holdout-v1.json 留出子集 + 宿主边界断言 A1–A8 全绿）须在 v6.3 功能代码合并前完成——治理文本修订（§八 AGENTS.md 修订稿）与红队回归为下一步执行项 | 维护者 |
